    ...
    protected function initButtons()
    {
        ...
        if (!isset($this->buttons['delete'])) {
            $this->buttons['delete'] = [
                ...
                'options' => [
                    'class' => 'btn-default',
                    'title' => Yii::t('adminlte', 'Delete'),
                    'data-confirm' => Yii::t('adminlte', 'Are you sure you want to delete this item?'),
                    'data-method' => 'delete'
                ]
            ];
        }
        ...
        if (!isset($this->buttons['batch-delete'])) {
            $this->buttons['batch-delete'] = [
                ...
                'options' => [
                    'id' => 'batch-delete',
                    'class' => 'btn-default',
                    'title' => Yii::t('adminlte', 'Delete selected')
                ]
            ];
        }
        ...
        if (strpos($this->buttonsTemplate, '{batch-delete}') !== false && $this->grid !== null && isset($this->buttons['batch-delete'])) {
            ...
            $view->registerJs(
                "jQuery(document).on('click', '#batch-delete', function (evt) {" .
                    ...
                    "if (keys == '') {" .
                        "alert('" . Yii::t('adminlte', 'You need to select at least one item.') . "');" .
                    "} else {" .
                        "if (confirm('" . Yii::t('adminlte', 'Are you sure you want to delete selected items?') . "')) {" .
                            "jQuery.ajax({" .
                                "type: 'POST'," .
